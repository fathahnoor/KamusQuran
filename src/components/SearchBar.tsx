...
export function SearchBar({
  ...
  const handleVoice = () => {
    ...
    <div className="space-y-3">
      {/* Search input row: full-width on mobile, side-by-side on laptop */}
      <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className={`w-full rounded-xl border border-ink-200 bg-ink-50/50 px-4 py-3.5 text-base text-ink-900 shadow-sm transition-all placeholder:text-ink-400 focus:border-accent-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-accent-500/20 ${
              isArabicInput ? "font-arabic text-xl !pr-4 !pl-12" : "font-sans !pl-11"
            }`}
            aria-label="Input pencarian"
          />
          {voice.interimTranscript && (
            <div className="absolute right-3 top-1/2 max-w-[60%] -translate-y-1/2 truncate text-sm italic text-ink-400">
              {voice.interimTranscript}
            </div>
          )}
        </div>

        {/* Action buttons: Cari (left) then Mic (right) — mic is oversized for thumb access */}
        <div className="flex items-center gap-2 sm:gap-2.5">
          <button
            onClick={onSubmit}
            className="flex h-14 flex-1 shrink-0 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-accent-600 to-accent-500 px-6 text-base font-semibold text-white shadow-sm shadow-accent-500/30 transition-all hover:shadow-md hover:shadow-accent-500/40 active:scale-[0.98] sm:h-12 sm:flex-none"
          >
          ...
          {voice.supported && (
            <button
              ...
              className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-xl shadow-sm transition-all active:scale-95 sm:h-14 sm:w-14 ${
                voice.listening
                  ? "bg-red-500 text-white animate-pulse shadow-red-500/30"
                  : "bg-ink-100 text-ink-600 hover:bg-ink-200 hover:shadow-md"
              }`}
