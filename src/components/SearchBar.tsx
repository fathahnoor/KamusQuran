      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <input
            ...
            className={`w-full rounded-lg border border-ink-300 bg-white px-4 py-3 text-base text-ink-900 shadow-sm transition-colors placeholder:text-ink-400 focus:border-accent-500 focus:outline-none focus:ring-2 focus:ring-accent-500/20 ${
              isArabicInput ? "font-arabic text-xl" : "font-sans"
            }`}
            aria-label="Input pencarian"
          />
          {voice.interimTranscript && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-sm italic text-ink-400">
              {voice.interimTranscript}
            </div>
          )}
        </div>
        ...
        <button
          onClick={onSubmit}
          className="shrink-0 rounded-lg bg-ink-800 px-5 py-3 text-sm font-semibold text-ink-50 transition-colors hover:bg-ink-900"
        >
          Cari
        </button>
      </div>
