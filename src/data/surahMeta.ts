...
// Full 114-surah metadata for occurrence display and ayah lookups.
export const SURAHS: SurahMeta[] = [
  ...
  { number: 40, name: "غافر", englishName: "Ghaafir", englishNameTranslation: "The Forgiver", numberOfAyahs: 85, revelationType: "Meccan" },
  ...
  { number: 86, name: "الطارق", englishName: "At-Taariq", englishNameTranslation: "The Morning Star", numberOfAyahs: 19, revelationType: "Meccan" },
...
/** Quick lookup map by surah number. */
export const SURAH_MAP: Map<SurahNumber, SurahMeta> = new Map(
  SURAHS.map((s) => [s.number, s])
);

/** Format a surah:ayah reference for display, e.g. "Al-Baqara (2):255". */
export function formatSurahAyah(surah: SurahNumber, ayah: number): string {
  const meta = SURAH_MAP.get(surah);
  return `${meta?.englishName ?? `Surah ${surah}`} (${surah}):${ayah}`;
}
